...
def load_adv_dataset() -> Dict[str, Any]:
    ...
    detail = {n: (list(map(str, d.columns)) if d is not None else m)
              for (n, d, m) in frames}
    raise AdvDataError(
        "No file in the SEC archive had firm name, state and CRD together.",
...
def fetch_firm_raum(crd: str, client: "httpx.Client") -> tuple:
    ...
    try:
        ...
        if resp.status_code != 200:
            return 0.0, f"ADV fetch HTTP {resp.status_code}"
...
class CuratorAgent:
    ...
    async def search_ria_universe(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        st.caption(
            f"{state_pool:,} advisers in selected states ({dataset['rows']:,} registered nationally)"
