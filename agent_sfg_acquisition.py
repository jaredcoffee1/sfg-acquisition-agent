        ...
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            ...
            for name in members[:8]:  # bound the work
                try:
                    ...
                    frames.append((name, None, {"_read_error": str(e)}))
                    continue
                frames.append((name, d, detect_adv_columns(list(d.columns))))
    else:
        d = pd.read_excel(io.BytesIO(content), engine="openpyxl")
        frames.append((found_url.rsplit("/", 1)[-1], d, detect_adv_columns(list(d.columns))))

    usable = [(n, d, m) for (n, d, m) in frames if d is not None]

    # Preferred: a single member carrying identity AND aum.
    complete = [(n, d, m) for (n, d, m) in usable
                if m.get("firm_name") and m.get("state") and m.get("aum")]
    if complete:
        name, df, mapping = complete[0]
        return {"df": df, "mapping": mapping, "source_url": found_url,
                "rows": len(df), "member": name}

    # Otherwise join the identity member to whichever member holds AUM, on CRD.
    ident = next(((n, d, m) for (n, d, m) in usable
                  if m.get("firm_name") and m.get("state") and m.get("crd")), None)
    aum_part = next(((n, d, m) for (n, d, m) in usable
                     if m.get("aum") and m.get("crd")), None)

    if ident and aum_part:
        (iname, idf, imap) = ident
        (aname, adf, amap) = aum_part
        left = idf.copy()
        right = adf[[amap["crd"], amap["aum"]]].copy()
        left["_crd_key"] = left[imap["crd"]].astype(str).str.strip()
        right["_crd_key"] = right[amap["crd"]].astype(str).str.strip()
        right = right.drop(columns=[amap["crd"]]).drop_duplicates(subset="_crd_key")
        merged = left.merge(right, on="_crd_key", how="left")
        mapping = dict(imap)
        mapping["aum"] = amap["aum"]
        return {"df": merged, "mapping": mapping, "source_url": found_url,
                "rows": len(merged), "member": f"{iname} + {aname}"}

    # Nothing worked — report exactly what is in the archive so the next step
    # is informed rather than another guess.
    detail = {n: (list(map(str, d.columns)) if d is not None else m)
              for (n, d, m) in frames}
    raise AdvDataError(
        "Could not find regulatory AUM in any file inside the SEC archive.",
        columns=[f"--- {n} ---" for n in detail] +
                [c for cols in detail.values() for c in (cols if isinstance(cols, list) else [])][:400],
