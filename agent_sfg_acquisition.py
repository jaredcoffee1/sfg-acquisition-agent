...
def load_adv_dataset() -> Dict[str, Any]:
    ...
    Cached for a week: the SEC publishes monthly, and the file is large enough
    that re-fetching per search would be wasteful. Returns a dict so failures
    carry a message instead of raising into the UI.
    """
    import io
    import zipfile
    ...
    try:
        with httpx.Client(timeout=120, follow_redirects=True, headers=headers) as client:
            ...
            # Find the newest non-exempt adviser file: ia<MMDDYYYY>...zip|xlsx
            links = re.findall(r'href="([^"]*ia\d{8}[^"]*\.(?:zip|xlsx))"', landing.text, re.IGNORECASE)
            links = [l for l in links if "exempt" not in l.lower()]
            if not links:
                return {"error": "Could not find an adviser data file link on the SEC page."}
        ...
        if missing:
            return {
                "error": f"Could not identify required columns {missing} in the SEC file.",
                "columns": list(df.columns)[:60],
                "mapping": mapping,
            }

        return {"df": df, "mapping": mapping, "source_url": newest, "rows": len(df)}

    except Exception as e:
        return {"error": f"SEC data load failed: {e}"}
        ...
        if "error" in dataset:
            ...
            if dataset.get("columns"):
                with st.expander("🔧 Column headings found in the SEC file", expanded=True):
                    ...
                    st.write("Detected mapping:", dataset.get("mapping"))
            return []
