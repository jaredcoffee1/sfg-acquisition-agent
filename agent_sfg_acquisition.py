...
class CuratorAgent:
    ...
    async def search_ria_universe(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        frame = frame.assign(_aum_m=aum_millions)

        missing_aum = int((frame["_aum_m"] <= 0).sum())
        in_range = frame[(frame["_aum_m"] >= aum_min) & (frame["_aum_m"] <= aum_max)]
        in_range = in_range.sort_values("_aum_m", ascending=False)
        ...
        st.caption(
            f"{len(in_range):,} firms fall inside ${aum_min:,.0f}M–${aum_max:,.0f}M "
            f"(ranked across the full state pool, not a sample); taking top {len(candidates)}."
            + (f" {missing_aum:,} advisers reported no AUM figure." if missing_aum else "")
        )
        return candidates
