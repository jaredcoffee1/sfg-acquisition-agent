...
ADV_COLUMN_PATTERNS = {
    ...
    # Item 5.D client-type breakdown. Letters are the ADV categories; (1) is a
    # client count and (3) is AUM from that type. VERIFIED against live rows:
    #   (a) individuals   (b) high-net-worth individuals
    #   (f) pooled investment vehicles  <- hedge / PE funds
    #   (g) pension and profit-sharing plans
    # This is what separates a wealth-management practice from a fund manager:
    # Sabrepoint Capital showed 0 individual clients and 100% pooled AUM, while
    # Elevated Capital showed 1,006 individual clients and 0 pooled.
    "indiv_aum":      [r"^5D\(a\)\(3\)$"],
    "hnw_aum":        [r"^5D\(b\)\(3\)$"],
    "pooled_aum":     [r"^5D\(f\)\(3\)$"],
...
def load_adv_dataset() -> Dict[str, Any]:
    ...
    def wanted_column(col: Any) -> bool:
        """Keep only the columns we map. The registered roster has 448 of them;
        loading all for ~15k advisers would be hundreds of MB on Railway."""
        text = str(col)
        for patterns in ADV_COLUMN_PATTERNS.values():
            for pattern in patterns:
    ...
    # filing itself. CRD is required here as the firm identifier.
...
class RIATarget:
    ...
    client_count: int = 0            # Individual + HNW individual clients
    retail_pct: float = 0.0          # % of AUM from individual clients
    pension_pct: float = 0.0         # % of AUM from pension/profit-sharing plans
...
class CuratorAgent:
    ...
    async def search_ria_universe(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        st.caption(
            f"Roster: {dataset.get('member', '?')} — "
            f"{state_pool:,} advisers in selected states of {dataset['rows']:,} in file"
            + (f"; {era_dropped:,} exempt-reporting dropped" if era_dropped else "")
        )

        # AUM comes straight from column 5F(2)(c) — total regulatory assets
        # under management, in whole dollars, as filed. No PDFs, no scraping,
        # no LLM anywhere near this number.
        aum_min = float(filters.get("aum_min", 0))
        aum_max = float(filters.get("aum_max", 10 ** 9))

        def dollars(col_key: str):
            col = cols.get(col_key)
            if not col or col not in frame.columns:
                return frame[cols["aum"]].map(lambda _: 0.0)
            return frame[col].map(adv_to_millions)

        def counts(col_key: str):
            col = cols.get(col_key)
            if not col or col not in frame.columns:
                return frame[cols["aum"]].map(lambda _: 0.0)
            return pd.to_numeric(
                frame[col].astype(str).str.replace(r"[, ]", "", regex=True),
                errors="coerce",
            ).fillna(0.0)

        aum_millions = frame[cols["aum"]].map(adv_to_millions)
        retail_m = dollars("indiv_aum") + dollars("hnw_aum")
        pension_m = dollars("pension_aum")
        pooled_m = dollars("pooled_aum")
        client_n = counts("indiv_clients") + counts("hnw_clients")

        safe_aum = aum_millions.where(aum_millions > 0, other=1.0)
        frame = frame.assign(
            _aum_m=aum_millions,
            _retail_pct=(retail_m / safe_aum * 100).clip(0, 100),
            _pension_pct=(pension_m / safe_aum * 100).clip(0, 100),
            _pooled_pct=(pooled_m / safe_aum * 100).clip(0, 100),
            _clients=client_n,
        )

        missing_aum = int((frame["_aum_m"] <= 0).sum())
        in_range = frame[(frame["_aum_m"] >= aum_min) & (frame["_aum_m"] <= aum_max)]
        before_shape = len(in_range)

        # Keep wealth-management practices, drop fund managers. Verified on live
        # data: hedge funds show 0 individual clients and near-100% pooled AUM.
        min_retail = float(filters.get("min_retail_pct", 50))
        min_clients = int(filters.get("min_clients", 25))
        in_range = in_range[
            (in_range["_retail_pct"] >= min_retail) & (in_range["_clients"] >= min_clients)
        ]
        dropped_funds = before_shape - len(in_range)

        rank_by = filters.get("rank_by", "AUM (highest)")
        sort_col = {
            "AUM (highest)": "_aum_m",
            "Most clients": "_clients",
            "Most retirement-focused": "_pension_pct",
        }.get(rank_by, "_aum_m")
        in_range = in_range.sort_values(sort_col, ascending=False)

        limit = int(filters.get("limit", 10))
        emp_col = cols.get("employees")

        candidates: List[Dict[str, Any]] = []
        for _, row in in_range.head(limit).iterrows():
            ...
            employees = str(row.get(emp_col, "") or "").strip() if emp_col else ""
            note_bits = [f"CRD {crd}"] if crd else []
            if employees and employees.lower() not in ("nan", "none"):
                note_bits.append(f"{employees} employees")

            candidates.append({
                "firm_name": str(row.get(cols["firm_name"], "") or "").strip(),
                "website": website,
                "city": str(row.get(cols["city"], "") or "").strip().title() if cols.get("city") else "",
                "state": str(row.get(cols["state"], "") or "").strip().upper() if cols.get("state") else "",
                "aum_millions": float(row["_aum_m"]),
                "client_count": int(row["_clients"]),
                "retail_pct": float(row["_retail_pct"]),
                "pension_pct": float(row["_pension_pct"]),
                "crd": crd,
        ...
        st.caption(
            f"{before_shape:,} firms inside ${aum_min:,.0f}M–${aum_max:,.0f}M; "
            f"{dropped_funds:,} dropped as fund managers "
            f"(under {min_retail:.0f}% individual-client AUM or under {min_clients} clients); "
            f"{len(in_range):,} qualifying practices. Ranked by {rank_by.lower()}; "
            f"taking top {len(candidates)}."
            + (f" {missing_aum:,} advisers reported no AUM." if missing_aum else "")
        )
        return candidates
    ...
    with st.sidebar:
        ...
        aum_min, aum_max = st.slider(
            ...
            (
                ...
                int(st.session_state.curator_filters.get("aum_max", 250))
            )
        )
        min_retail = st.slider(
            "Minimum % of AUM from individual clients",
            0, 100,
            int(st.session_state.curator_filters.get("min_retail_pct", 50)),
            help="Separates wealth-management practices from fund managers. "
                 "Hedge funds report 0% here; retail RIAs report 90%+.",
        )
        min_clients = st.number_input(
            "Minimum individual clients",
            min_value=0, max_value=5000,
            value=int(st.session_state.curator_filters.get("min_clients", 25)),
            step=5,
            help="Excludes family offices and single-fund shops with a handful of clients.",
        )
        rank_by = st.selectbox(
            "Rank by",
            ["AUM (highest)", "Most clients", "Most retirement-focused"],
            index=["AUM (highest)", "Most clients", "Most retirement-focused"].index(
                st.session_state.curator_filters.get("rank_by", "AUM (highest)")
            ),
            help="Ranking on AUM alone clusters every result at your ceiling.",
        )
        # The old "Firm Types" multiselect was removed. It looked like a filter
        # but was never applied to anything — a control that appears to narrow
        # results while doing nothing is worse than no control. The real screen
        # is the ADV client-mix filter above, which is measured, not declared.
        ...
        st.session_state.curator_filters = {
            ...
            "min_retail_pct": float(min_retail),
